function CreateConstellationWithPhaseTLE(satPerPlane,planeCount, fourierAnomolyCoeffs, a, i, fileToSave)
...
mean_anomoly_stagger_array = FourierSinInterpolation(fourierAnomolyCoeffs, 360*2, RAAN_array);
mean_anomoly_start_array = linspace(0,360-(360/satPerPlane),satPerPlane);
...
for j=1:length(RAAN_array)
    raan = RAAN_array(j);
    for m = 1:length(mean_anomoly_start_array)
        anomoly = mean_anomoly_start_array(m) + mean_anomoly_stagger_array(j);
